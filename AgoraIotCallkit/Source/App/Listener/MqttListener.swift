...
class MqttListener : FsmMqtt.IListener{
    ...
    func do_INITMQTT(_ srcState: FsmMqtt.State) {
        ...
        let deviceAlias = app.context.aglab.session.userName
        let ret = app.proxy.mqtt.initialize(thingName:gwsess.cert.thingName, endPoint: gwsess.endPoint, token:  gwsess.pool_token,identityId: gwsess.pool_identityId,identityPoolId: gwsess.pool_identityPoolId,region: gwsess.region,appId: app.config.appId,deviceAlias: deviceAlias)
